...
from odoo.tools import float_compare # type: ignore

class PurchaseOrder(models.Model):
    _inherit = "purchase.order"
    
    account_payment_ids = fields.One2many(
        "account.payment", "purchase_id", string="Pay Purchase Advance"
    )
    
    amount_residual = fields.Float(
        "Residual Amount", readonly=True, compute="_compute_purchase_advance_payment", store=True
    )
    
    payment_line_ids = fields.Many2many(
        "account.move.line", string="Payment Move Lines", compute="_compute_purchase_advance_payment", store=True
    )
    ...
    def _compute_purchase_advance_payment(self):
        ...
        for order in self:
            mls = order.account_payment_ids.mapped("move_id.line_ids").filtered(
                lambda x: x.account_id.account_type == "liability_payable" and x.parent_state == "posted"
            )
