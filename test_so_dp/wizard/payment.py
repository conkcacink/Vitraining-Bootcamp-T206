...
class AccountVoucherWizard(models.TransientModel):
    ...
    currency_amount = fields.Monetary(
        "Currency Amount", readonly=True, currency_field="currency_id"
    )
    
    payment_ref = fields.Char("Ref.")
    
    payment_type = fields.Selection(
        [("inbound","Inbound"),("outbound", "Outbound")], default="inbound", required=True,
    )
    
    @api.depends("journal_id")
    def _computer_get_journal_currency(self):
        for wzd in self:
    ...
    def check_amount(self):
        ...
        if self.env.context.get("active_id", False):
            ...
            if self.payment_type == "inbound":
                if(
                    float_compare(
                        self.currency_amount, self.order_id.amount_residual, precision_digits = 2,
                    ) > 0
                ):
                    raise exceptions.ValidationError(
                        _("Inbound amount of advance is greater than residual amount on sale.")                    
                )
            else:
                paid_in_advance = self.order_id.amount_total - self.amount_total
                
                if(
                    float_compare(
                        self.currency_amount, paid_in_advance, precision_digits = 2,
                    ) > 0
                ):
                    raise exceptions.ValidationError(
                        _(
                            "Outbound amount of advance is greater than the paid advance amount"
                        )
                    )
    ...
    def _prepare_payment_vals(self, sale):
        ...
        if self.amount_advance < 0.0:
            raise UserError(
                _(
                    "The amount to advance must always be positive. Please use the payment type to indicate if this is an inbound or an outbound payment."
                )
            )
        return {
            "date" : self.date,
